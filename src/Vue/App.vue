...
const error = ref('')
const status = ref('Loading available currencies...')
...
function syncAmountInput() {
	amountInput.value = formatCurrencyInput(amount.value, fromCurrency.value)
}

function handleAmountInput(event: Event) {
	const inputElement = event.currentTarget as HTMLInputElement
	amount.value = parseCurrencyAmount(inputElement.value)
	syncAmountInput()
	void refreshRate()
}
...
function handleAmountKeydown(event: KeyboardEvent) {
	...
	if (amount.value === 0 && (event.key === 'Backspace' || event.key === 'Delete')) {
...
function pickDefaultCurrencies(options: CurrencyOption[]) {
	...
	fromCurrency.value = codes.includes('USD') ? 'USD' : codes[0] ?? 'USD'
	toCurrency.value = codes.includes('EUR') ? 'EUR' : codes.find((code) => code !== fromCurrency.value) ?? fromCurrency.value

	if (fromCurrency.value === toCurrency.value && codes.length > 1) {
		toCurrency.value = codes.find((code) => code !== fromCurrency.value) ?? toCurrency.value
	}
...
async function refreshRate() {
	...
	try {
		const nextRate = await getRate(fromCurrency.value, toCurrency.value)
		...
		rate.value = nextRate
		status.value = `${formatCurrencyAmount(1, fromCurrency.value)} = ${formatCurrencyAmount(nextRate, toCurrency.value)}`
	} catch (thrownError) {
		if (currentRequest !== requestId.value) {
			return
		}

		error.value = thrownError instanceof Error ? thrownError.message : 'Unable to load the exchange rate.'
		status.value = 'Could not load the current exchange rate.'
	} finally {
		if (currentRequest === requestId.value) {
...
async function loadCurrencies() {
	...
	try {
		...
		currencies.value = Object.entries(data)
			.map(([code, name]) => ({ code, name, symbol: getCurrencySymbol(code) }))
			.sort((left, right) => left.code.localeCompare(right.code))
		pickDefaultCurrencies(currencies.value)
		await refreshRate()
	} catch (thrownError) {
		error.value = thrownError instanceof Error ? thrownError.message : 'Unable to load currencies.'
		status.value = 'Unable to load currencies right now.'
	} finally {
		...
		<section class="converter-card" :aria-busy="loadingCurrencies || loadingRate">
			<div class="controls">
				...
				<div class="currency-grid">
					<label>
						<span>From</span>
						<select v-model="fromCurrency" @change="handleFromCurrencyChange" :disabled="loadingCurrencies">
					...
					<button class="swap" type="button" @click="swapCurrencies" aria-label="Swap currencies">
						⇄
					</button>

					<label>
						<span>To</span>
						<select v-model="toCurrency" @change="refreshRate" :disabled="loadingCurrencies">
							<option v-for="currency in currencies" :key="currency.code" :value="currency.code">
								{{ currency.code }} - {{ currency.name }}
							</option>
						</select>
				...
				<div class="status-row">
					...
					<button type="button" class="refresh" @click="refreshRate" :disabled="loadingCurrencies || loadingRate">
						Refresh rate
					</button>
				</div>
